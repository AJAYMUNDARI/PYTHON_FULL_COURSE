...
mycursor=mydb.cursor(buffered=True) #to access all the sql statement
...
print("3. Search Record Details as per the account
number".center(140))
print("4. Update Record".center(140))
print("5. Delete Record".center(140))
print("6. TransactionsDebit/Withdraw from the
account".center(140))
...
Rec=[Acc,Name.upper(),Mob,email.upper(),Add.upper(),City.upper(),Coun
try.upper(),Bal]
...
if ch=='N' or ch=='n':
break
def DispSortAcc(): #Function to Display records as per
...
print("Table doesn't exist")
def DispSearchAcc(): #Function to Search for the Record from the File
...
print("No such Table")
def Debit(): #Function to Withdraw the amount by assuring the min
balance of Rs 5000
try:
cmd="select * from BANK"
mycursor.execute(cmd)
S=mycursor.fetchall()
print("Please Note that the money can only be debited if
min balance of Rs 5000 exists")
acc=input("Enter the account no from which the money is
to be debited")
for i in S:
i=list(i)
if i[0]==acc:
Amt=float(input("Enter the amount to be
withdrawn"))
if i[7]-Amt>=5000:
i[7]-=Amt
cmd="UPDATE BANK SET BALANCE=%s WHERE
ACCNO=%s"
val=(i[7],i[0])
mycursor.execute(cmd,val)
mydb.commit()
...
print("Table Doesn't exist")
def Credit(): #Function to Withdraw the amount by assuring the min
balance of Rs 5000
try:
cmd="select * from BANK"
mycursor.execute(cmd)
